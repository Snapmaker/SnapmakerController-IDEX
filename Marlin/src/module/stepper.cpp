...
block_t* Stepper::new_current_block = nullptr; // (= nullptr) A pointer to the block currently being traced
...
uint32_t Stepper::block_phase_isr() {
  ...
  if (new_current_block == nullptr) {
    new_current_block = planner.get_current_block();

    if (new_current_block == nullptr) {
      return interval;
    }
    
    if(new_current_block->shaper_data.is_zero_speed) {
      new_current_block = nullptr;
      planner.release_current_block();
      return 0;
    }

    block_print_time = new_current_block->shaper_data.last_print_time;
    Move& end_move = moveQueue.moves[new_current_block->shaper_data.move_end];
