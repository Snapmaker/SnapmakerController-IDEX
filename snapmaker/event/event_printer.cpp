...
static uint16_t load_gcode_file_info(uint8_t *buf) {
  uint16_t ret_len = 0;
  uint8_t temp_len = 0;
  uint8_t * addr = print_control.get_file_md5(temp_len);
  ...
  buf += (temp_len + 2);
  addr = print_control.get_file_name(temp_len);
  if (addr) {
    memcpy(buf+2, addr, temp_len);
    *((uint16_t *)buf) = temp_len;
    ret_len += temp_len + 2;
...
static ErrCode request_start_work(event_param_t& event) {
  ...
  if (result == E_SUCCESS) {
    ...
    print_control.set_file_md5(data, data_len);
    data = event.data + data_len + 2;
    data_len = event.data[data_len + 1];
    print_control.set_file_name(data, data_len);
...
static ErrCode request_power_loss_status(event_param_t& event) {
  uint16_t ret_len = load_gcode_file_info(event.data+1);
  SERIAL_ECHOLNPAIR("SC req power loos status:", !!ret_len);
  if (ret_len) {
    event.data[0] = E_SUCCESS;
    event.length = ret_len;
  } else {
    event.data[0] = PRINT_RESULT_NO_PL_DATA_E;
    event.length = 5;
  }
  return send_event(event);
}
...
event_cb_info_t printer_cb_info[PRINTER_ID_CB_COUNT] = {
  {PRINTER_ID_REQ_FILE_INFO       , EVENT_CB_TASK_RUN, request_file_info},
  ...
  {PRINTER_ID_REQ_PL_STATUS       , EVENT_CB_TASK_RUN, request_power_loss_status},
  ...
  {PRINTER_ID_REQ_AUTO_PARK_STATUS, EVENT_CB_TASK_RUN, request_auto_pack_status},
  {PRINTER_ID_SET_AUTO_PARK_STATUS, EVENT_CB_TASK_RUN, set_auto_pack_mode},
  {PRINTER_ID_REQ_LINE            , EVENT_CB_TASK_RUN, request_cur_line},
};
